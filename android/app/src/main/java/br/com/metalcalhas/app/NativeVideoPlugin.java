...
public class NativeVideoPlugin extends Plugin {
    ...
    private void captureResult(PluginCall call, ActivityResult result) {
        ...
        String path = data.getStringExtra(NativeVideoActivity.EXTRA_VIDEO_PATH);
        if (path == null) {
            call.reject("O vídeo gravado não foi encontrado.");
            return;
        }
        ...
        response.put("mimeType", "video/mp4");
    ...
    public void uploadVideo(PluginCall call) {
        ...
        if (path == null || url == null || token == null) {
            call.reject("Dados incompletos para enviar o vídeo.");
            return;
        }
    ...
    private File validateTemporaryFile(String path) throws IOException {
        ...
        if (!file.getPath().startsWith(cache.getPath() + File.separator) ||
            ...
            throw new IOException("Arquivo temporário de vídeo inválido.");
        }
    ...
    private JSObject upload(File file, String targetUrl, String token) throws IOException {
        String boundary = "MetalCalhas-" + UUID.randomUUID();
        String prefix = "--" + boundary + "\r\n" +
            "Content-Disposition: form-data; name=\"Video\"; filename=\"" + file.getName() + "\"\r\n" +
        ...
        try {
            try (DataOutputStream output = new DataOutputStream(connection.getOutputStream());
                ...
                while ((read = input.read(buffer)) != -1) {
                    ...
                    notifyListeners("videoUploadProgress", progress);
                }
            ...
            if (status < 200 || status >= 300) {
                throw new IOException("Falha ao enviar vídeo (HTTP " + status + "). " + responseBody);
            }
